...
something else. A string that is not valid UTF-8 is an error: YAML
escapes name characters, not bytes, so such a value has no spelling.
...
- An embedded struct with no tag contributes its fields directly, and a
  shallower field wins over a deeper one of the same name.
- Key lookup is exact first, then case-insensitive.
- Keys the struct does not know are ignored.
